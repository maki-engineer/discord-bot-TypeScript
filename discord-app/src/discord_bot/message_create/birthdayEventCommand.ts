 ...
 * @param {string[]} commandList [月, 日, 時, 追加の文章]
 * @param {number} setTimeoutSec
 *
 * @return {Promise<boolean>}
 */
const isValidBirthdayCommandArgs = async (
  message: Message,
  commandList: string[],
  setTimeoutSec: number,
) => {
 ...
 * @return {Promise<boolean>}
 */
const validateBirthdayArgsNumeric = async (
  message: Message,
  commandList: string[],
  setTimeoutSec: number,
) => {
 ...
 * @return {Promise<boolean>}
 */
const validateBirthdayDate = async (
  message: Message,
  commandList: string[],
  setTimeoutSec: number,
) => {
 ...
 *
 * @param {string[]} commandList [月, 日, 時, 追加の文章]
 * @param {Date} eventDateTime イベントの日付
 *
 * @return {Promise<string>}
 */
const createEventText = async (commandList: string[], eventDateTime: Date) => {
...
export default async (
  ...
  if (
    !(await isValidBirthdayCommandArgs(message, commandList, setTimeoutSec)) ||
    !(await validateBirthdayArgsNumeric(message, commandList, setTimeoutSec)) ||
    !(await validateBirthdayDate(message, commandList, setTimeoutSec))
  ) {
    return;
  }
  ...
  const eventText = await createEventText(commandList, eventDateTime);
