...
export default class VoiceVox {
   ...
   * @return {void}
   */
  addWavFileToQueue(wavFile: string): void {
    this.discordBot.wavFileQueue.push(wavFile);

    if (!this.discordBot.isPlaying) {
      this.playNext();
    }
  }

  /**
   * キューに追加されたwavファイルを順番に読み上げ
   *
   * @return {void}
   */
  private playNext(): void {
    if (this.discordBot.wavFileQueue.length === 0) {
      this.discordBot.isPlaying = false;
   ...
   * @return {void}
   */
  playDisconnectAnnounce(wavFile: string): void {
    this.discordBot.audioPlayer.stop(true);
   ...
   * @return {string}
   */
  static formatMessage(messageContent: string): string {
  ...
  /**
   * メッセージの中に単語辞書に含まれている単語があったら、登録されている読み方に置き換える
   *
   * @param {string} messageContent フォーマット対象のメッセージ
   *
   * @return {void}
